...
const UploadItemModal = ({ user }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.current.checkValidity()) {
      ...
      ItemService.upload(user.jwt, formData)
        .then((res) => {
          switch (res.status) {
            case 200:
              setLoading(false);
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title" id="uploadItemModalLabel">
              Upload Item
            </h5>
            <button
              type="button"
            ...
            <div className="mb-2">
              {loading && (
                <div className="hstack align-items-center justify-content-center">
                  <div className="spinner-border" role="status">
                    <span className="visually-hidden">Uploading items...</span>
                  </div>
                  <strong className="ms-2">Uploading items...</strong>
                </div>
              )}
              {error && (
              ...
              {status && (
                <div className="hstack align-items-center justify-content-center text-success">
                  ...
                  <strong className="ms-2">Upload Item Success</strong>
                </div>
              )}
            </div>
            <div>
              <ul className="list-unstyled">
                <li>
                  ...
                  <a
                    className="ms-1"
                    onClick={() =>
                      ItemService.downloadTemplate(user.jwt)
                        .then((res) => res.blob())
                        .then((data) =>
                          fileDownload(data, "items-template.xlsx")
                        )
            ...
            <form ref={form} onSubmit={handleSubmit} noValidate>
              <div className="input-group">
                <input
                  ...
                  id="uploadItemFile"
                  aria-describedby="uploadItemFile"
              ...
              <div className="form-check mt-2">
                <input
                  ...
                  id="uploadItemOverwrite"
                />
                <label
                  className="form-check-label"
                  htmlFor="uploadItemOverwrite"
                >
                  Overwrite?
                </label>
              </div>
...
export default connect(mapStateToProps, {})(UploadItemModal);
