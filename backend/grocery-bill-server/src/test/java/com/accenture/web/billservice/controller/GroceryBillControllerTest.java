package com.accenture.web.billservice.controller;

import static org.mockito.Mockito.*;

import com.accenture.web.billservice.domain.*;
import com.accenture.web.billservice.exception.AppException;
import com.accenture.web.billservice.proxy.ItemFeignClient;
import com.accenture.web.billservice.service.ExcelFileService;
import com.accenture.web.billservice.service.GroceryBillServiceImpl;
...
class GroceryBillControllerTest {
    ...
    // TODO Find way to make service.addGroceryBill Mock work without Mockito.any()
    @Test
    @DisplayName("Create Existing Bill")
    void createGroceryBill_withExistingBill_returnBadRequest() throws Exception {
        // Arrange
        GroceryBill newBill = new DiscountedBill(clerk);
        newBill.setItemList(items);
        newBill.getTotalBill();
        ...
        when(service.addGroceryBill(Mockito.any())).thenThrow(new AppException("Bill of same Bill Id already exist", HttpStatus.BAD_REQUEST));
